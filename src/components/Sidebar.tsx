...
export function Sidebar() {
  ...
  return (
    <aside className="flex h-full w-72 shrink-0 flex-col border-r border-ink-700 bg-ink-900/80 backdrop-blur">
      <div className="border-b border-ink-700 px-4 py-4">
        <h1 className="font-display text-xl font-bold tracking-wide text-gold-400">Skill Tree</h1>
        <p className="mt-0.5 text-xs text-ink-400">Your quest log for growth</p>
      </div>
          ...
          return (
            <div key={area.id} className="mb-2">
              <button
                ...
                className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-xs font-bold uppercase tracking-wider text-ink-400 hover:bg-ink-800"
              >
                <span>{area.icon}</span>
                    ...
                    return (
                      <button
                        ...
                        className={`group relative flex flex-col gap-1.5 rounded-lg border px-3 py-2 text-left transition-colors ${
                          active
                            ? 'border-gold-500/60 bg-ink-800'
                            : 'border-transparent bg-ink-850/40 hover:border-ink-600 hover:bg-ink-850'
                        }`}
                      >
                        <div className="flex items-center gap-2">
                          <span className="text-base leading-none">{tree.icon}</span>
                          <span className="flex-1 truncate text-sm font-semibold text-ink-100">{tree.name}</span>
                          <span
                            onClick={(e) => {
                              e.stopPropagation()
                              if (confirm(`Delete "${tree.name}" and all its nodes?`)) deleteTree(tree.id)
                            }}
                            className="hidden shrink-0 text-ink-500 hover:text-ember-400 group-hover:block"
                            title="Delete path"
                          >
                            ✕
                          </span>
      ...
      <div className="border-t border-ink-700 p-3">
        <button
          ...
          className="w-full rounded-lg border border-gold-500/50 bg-gold-500/10 px-3 py-2 text-sm font-semibold text-gold-300 transition-colors hover:bg-gold-500/20"
        >
          + New Learning Path
        </button>
      </div>
